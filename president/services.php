				...
				<div class="card">
					...
					<div class="card-block">
						...
						<ul class="nav nav-tabs">
							...
							<li class="nav-item">
								<a class="nav-link" data-toggle="tab" href="#menu1">My Requests</a>
							</li>
							<li class="nav-item">
								<a class="nav-link" data-toggle="tab" href="#menu2">All Requests</a>
							</li>
						</ul>
						<!-- Tab panes -->
						<div class="tab-content">
							<div class="tab-pane active container" id="home" style="padding: 20px;">
								<form class="form-material" id="addfrm" method="post" action="php/servicesadd.php">
							...
							<div class="tab-pane container" id="menu1" style="padding: 20px;">
								<div class="table-responsive">
									...
									$res=$ob->db_read($sql);
									?>
									<table class="table" id="reqtable1">
										<thead>
											<tr>
												<th>#</th>
												<th>Subject</th>
							...
							<div class="tab-pane container" id="menu2" style="padding: 20px;">
								<div class="table-responsive">
									<?php
									$sql="select jagratha.status as jstatus,jagratha.*,reg.* from jagratha,reg where reg.username=jagratha.user order by jagratha.status";
									$res=$ob->db_read($sql);
									?>
									<table class="table" id="reqtable2">
										<thead>
											<tr>
												<th>#</th>
												<th>User</th>
											...
											while($row=mysqli_fetch_array($res)) {
												...
												<tr>
													...
													<td>
														...
														if($row['jstatus']==0) {
															?>
															<button type="button" onclick="frwd('<?php echo $row['requestid']?>')" class="btn btn-sm btn-success">Forward to ADS</button>
															<?php
														}
														if($row['jstatus']==1)
															echo "Forwarded to ADS";
		...
		function frwd(reqid) {
			Swal.fire({
				...
				if (result.isConfirmed) {
					$.ajax({
						type: 'post',
						url:'php/servicesfrwd.php',
						data: {'frwd':'set','reqid':reqid},
						success: function(ret){
							if(ret) {
								Swal.fire({
									text: "Request Forwarded Successfully",
									icon: 'success',
									showClass: {popup: 'animated fadeInDown faster'},
									hideClass: {popup: 'animated zoomOut faster'},
								}).then(() => {							
